...
import { convertRoutes2String } from './resolver/convertRoutes2String'
import { getFileStructure } from './resolver/getFileStructure'
import { getRoutesFromFileStructure } from './resolver/getRoutesFromFileStructure'

export type { RouteMap } from './resolver/convertRoutes2String'
export { createReactRoutes } from './utils/createReactRoutes'
...
export default function PagesPlugin(options: PagesPluginOptions = {}) {
  ...
  function getComputedRoutes(ctx: PageContext) {
    ...
    paths.forEach((path) => {
      if (path.includes("'"))
        throw new Error(`${path}文件路径不能包含单引号(')`)
    })
